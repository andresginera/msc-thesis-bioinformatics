...
performance. Nowadays, almost all computers have multicore processors but GAUDImm
uses only one core to execute a job, so we thought in a parallel computing
approach for the performance optimization. 
...
But we also thought in another way of implementation, an *external*
parallelization, consisting of a code that manually divides the main GAUDImm
process in subprocesses for then gathers the solutions and generates a unique
output.
...
To achieve this aim, it is necessary to perform a serialization process in
...
and they can send information between them. 
...
GAUDImm solutions are built over Chimera classes, which rely in the core
...
isolation technique, in which the subprocesses were totally independent.
...
begin with a unique initial population and will have an unequalled evolutionary
...
in a Python dictionary object. If the user doesn't specify to not use the
simplification of the subprocesses, the data variable is modified to decrease
...
``multiprocessing.Pool.map_async`` which receives a function, the execution
``gaudi run``, and an iterable, the list of names of the new files. 
...
completion of the subprocesses. Then, compare the 3D structure of one solution
...
information in a Python dictionary object. 

For creating the unique ``.gaudi-log`` file, PGaudi merge with a simple function
using basic methods for read and write files. 
...
After this command a process of PGaudi and its subprocesses will be generated.
...
The API documentation is as well in the Appendix section.